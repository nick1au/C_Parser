...
{
	int old_tracker = tracker;
	if(auto subTreeP = tokenObjectPs[tracker]->process_declaration()) // type_spec?
		return subTreeP;
	if (auto test_subTreeP = tokenObjectPs[tracker+1]->is_postfix_operator())
	{
		auto subTreeP = tokenObjectPs[tracker+1]->process_postfix(); // postfix?
		return subTreeP;
	}

	if ( (tokenObjectPs[tracker+3]->has_string_value("+")) ||(tokenObjectPs[tracker+3]->has_string_value("-")) ) // additive oeprator?
	{
		auto subTreeP = tokenObjectPs[tracker+3]->process_primary_exp();
		return subTreeP;
	}
	else if (tokenObjectPs[tracker+1]->has_string_value(">>")|| tokenObjectPs[tracker+1]->has_string_value("<<")) // shift operator?
	{
		auto subTreeP = tokenObjectPs[tracker+1]->process_primary_exp();
	}
